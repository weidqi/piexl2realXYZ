...
為了直接落地上述方案，倉庫提供了一組可插拔的 Python 模組：

| 路徑 | 功能概要 |
| --- | --- |
| `geo/dsm_io.py` | DSM 資料結構、仿射變換、雙線性插值讀寫。 |
| `geo/camera_model.py` | 相機內外參封裝，像素→射線、投影等。 |
| `geo/dsm_raycaster.py` | 以步進+二分的方式將 DSM 投射為相機深度圖。 |
| `depth/depth_pro_wrapper.py` | 封裝 Depth Pro 推理 callable。 |
| `fusion/region_masks.py` | 基於 DSM 的簡易地面/屋頂/天空/植被掩膜生成。 |
| `fusion/error_model.py` | 全局/分塊線性校正與低頻殘差平滑。 |
| `fusion/depth_fusion.py` | 分區域融合 DSM 與校正後深度，保留高頻細節。 |
| `fusion/multiframe.py` | 多幀融合（中位數/均值）。 |
| `build_static_background_depth.py` | 串接以上模組的一站式流程函式。 |
...
from build_static_background_depth import FusionConfig, run_pipeline
from depth.depth_pro_wrapper import DepthProWrapper
from geo.camera_model import CameraModel, make_intrinsics
from geo.dsm_io import DsmGrid, load_dsm_from_json

# 1) 準備相機與 DSM
K = make_intrinsics(fx, fy, cx, cy)
cam = CameraModel(K=K, R=R_wc, C_w=C_w)
dsm = load_dsm_from_json("demo_dsm.json")

# 2) 構造 Depth Pro wrapper（這裡用自定義推理函式）
def infer_depth(image_rgb, f_px):
    return depth_pro_model.predict(image_rgb, f_px)

depth_pro = DepthProWrapper(infer_fn=infer_depth)

# 3) 執行多幀融合
depth_map = run_pipeline(images, cam, dsm, depth_pro, FusionConfig())
```

程式中生成的 `depth_map` 為相機坐標系下的靜態背景深度，可直接供後續三維重建或稠密配準使用。
